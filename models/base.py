...
class BaseLearner(object):
    def __init__(self, args):
        ...
        if args['dataset'] in ['core50', 'domainnet']:
            self.is_domain = True
            self.class_num = 50 if args['dataset']=='core50' else 200
        ...
        task_num = 1+(self._memory_size//self._memory_per_class-args["init_cls"])//args["increment"]
        self.acc_matrix = np.zeros((task_num, task_num)) # 计算遗忘率
    @property
    ...
    def _evaluate(self, y_pred, y_true):
        ret = {}
        if self.is_domain:
            grouped = accuracy_domain(y_pred.T[0], y_true, self._known_classes, self.increment, class_num=self.class_num)
        else:
    ...
    def eval_task(self):
        ...
        if hasattr(self, '_class_means'):
            ...
            nme_forgetting, nme_backward = self._calculate_forget(nme_accy['grouped'])
            nme_accy['forget'], nme_accy['backward'] = nme_forgetting, nme_backward
        else:
            nme_accy = None

        return cnn_accy, nme_accy

    def _calculate_forget(self, acc):
        tup = [(key, value) for key, value in acc.items()][1:-2]
        task_id = len(tup)-1 # 0,1,2,3,...
        for i in range(len(tup)):
            self.acc_matrix[i, task_id] = tup[i][1]
        diagonal = np.diag(self.acc_matrix)
        forgetting, backward = -1, -1
        if task_id > 0:
            forgetting = np.mean((np.max(self.acc_matrix, axis=1) - self.acc_matrix[:, task_id])[:task_id])
            backward = np.mean((self.acc_matrix[:, task_id] - diagonal)[:task_id])
        return np.round(forgetting, decimals=2), np.round(backward, decimals=2)
        # return 0, 0
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            exemplar_vectors = []
            m = ori_m
            if len(data) < ori_m: # 当前类别样本数量小于要保存的样本数
                m = len(data)
            task_saved_samples_num += m
            for k in range(1, m+1):
                S = np.sum(exemplar_vectors, axis=0)  # [feature_dim] sum of selected exemplars vectors
                mu_p = (vectors + S) / k  # [n, feature_dim] sum to all vectors
                i = np.argmin(np.sqrt(np.sum((class_mean - mu_p) ** 2, axis=1)))
            ...
            _class_means[class_idx, :] = mean
        logging.info('Current Task saved {} samples in total.'.format(task_saved_samples_num))
