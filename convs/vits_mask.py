...
from utils.quantizer import FakeLinearQuantizationFunction, quantize
from utils.toolkit import GetSubnetUnstructured
class ALinear(nn.Linear):
    def __init__(self, in_dim, out_dim, bias=True, mask=1, weight_bits=16, warmup_step=0):
        super(ALinear, self).__init__(in_dim, out_dim, bias)
        self.mask = mask
        # if self.mask < 1:
        ...
        self._fake_quantize = FakeLinearQuantizationFunction.apply
    ...
    def forward(self, input):
        # self.w = self.weight
        if self.training:
            self.w = self._fake_quantize(self.weight, self.weight_bits)
        else:
            ...
            self.w = self.weight_norm * self.quantized_weight
        # pdb.set_trace()
        if self.mask < 1:
            self.adj = GetSubnetUnstructured.apply(self.weight_mask.abs(), self.mask).to(self.weight.device)
            self.w1 = self.adj * self.w
            output = F.linear(input, self.w1, self.bias)
            return output
        else:
...
class Mlp(nn.Module):
    def __init__(
        ...
        self.fc1 = ALinear(in_features, hidden_features, mask=mask)
        self.act = act_layer()
        self.fc2 = ALinear(hidden_features, out_features, mask=mask)
...
class VisionTransformer(nn.Module):
    ...
    def reset_data_mask(self, train_data_num, seed=1993):
        torch.manual_seed(seed)
        # self.data_mask = torch.nn.Parameter(0.8+0.2*torch.rand((train_data_num)))
        self.data_mask = torch.nn.Parameter(torch.ones(train_data_num))
        # self.data_mask = torch.nn.Parameter(torch.zeros(train_data_num))
...
@register_model
def vit_base_patch16_224_in21k_our(pretrained=False, adapter=False, **kwargs):
